...
# ---------------------------------------------------------------------------
# Anti-brute-force équivalent pour le code à 4 chiffres des technicien·nes
# (même principe que pour l'admin, mais suivi par IP + nom visé).
# ---------------------------------------------------------------------------
PIN_MAX_ATTEMPTS = 5
...
def _pin_bucket_key(name):
    return _client_ip() + "|" + (name or "").strip().lower()


def _pin_blocked(name):
    entry = _pin_attempts.get(_pin_bucket_key(name))
    if not entry:
        return False
    count, first_fail = entry
    ...
    if time.time() - first_fail > PIN_LOCKOUT_SECONDS:
        _pin_attempts.pop(_pin_bucket_key(name), None)
        return False
    return True


def _register_pin_failure(name):
    key = _pin_bucket_key(name)
    count, first_fail = _pin_attempts.get(key, (0, time.time()))
    if time.time() - first_fail > PIN_LOCKOUT_SECONDS:
        count, first_fail = 0, time.time()
    _pin_attempts[key] = (count + 1, first_fail)


def _clear_pin_failures(name):
    _pin_attempts.pop(_pin_bucket_key(name), None)
...
def init_db():
    ...
    if USE_PG:
        ...
        try:
            cur.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_technicians_name_lower ON technicians (lower(name))")
        except Exception:
            conn.rollback()
        # Reprise des noms déjà utilisés dans les tickets existants : on crée une
        # fiche technicien (sans code, à définir par l'admin) pour chaque nom
        # distinct qui n'a pas encore de fiche, afin que rien ne soit perdu au
        # moment de l'activation des codes.
        try:
            cur.execute(
                """
                INSERT INTO technicians (id, name, pin_code, created_at)
                SELECT DISTINCT ON (lower(t.technicien)) md5(lower(t.technicien))::text, t.technicien, NULL, %s
                FROM tickets t
                WHERE t.technicien IS NOT NULL AND t.technicien <> ''
                  AND NOT EXISTS (
                    SELECT 1 FROM technicians x WHERE lower(x.name) = lower(t.technicien)
                  )
                ORDER BY lower(t.technicien), t.created_at ASC
                """,
        ...
        try:
            conn.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_technicians_name_lower ON technicians (name COLLATE NOCASE)")
        except Exception:
            pass
        # Reprise des noms déjà utilisés dans les tickets existants (voir
        # commentaire équivalent côté Postgres ci-dessus).
        existing_names = {row["name"].strip().lower() for row in conn.execute("SELECT name FROM technicians").fetchall()}
        seen_now = set()
        for row in conn.execute("SELECT DISTINCT technicien FROM tickets WHERE technicien IS NOT NULL AND technicien <> '' ORDER BY created_at ASC").fetchall():
            name = (row["technicien"] or "").strip()
            key = name.lower()
            if not name or key in existing_names or key in seen_now:
                continue
            seen_now.add(key)
            conn.execute(
                "INSERT INTO technicians (id, name, pin_code, created_at) VALUES (?, ?, NULL, ?)",
                (uuid.uuid4().hex, name, datetime.now(timezone.utc).isoformat()),
            )
        conn.commit()
    conn.close()
...
def get_technician_by_name(conn, name):
    name = (name or "").strip()
    if not name:
        return None
    if USE_PG:
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute("SELECT * FROM technicians WHERE lower(name) = lower(%s)", (name,))
        row = cur.fetchone()
        cur.close()
        return row
    return conn.execute("SELECT * FROM technicians WHERE lower(name) = lower(?)", (name,)).fetchone()


def verify_technician_pin(conn, name, pin):
    """Vérifie le code à 4 chiffres d'un·e technicien·ne. Retourne (ok, erreur)."""
    if _pin_blocked(name):
        return False, "Trop de tentatives, réessayez dans quelques minutes."
    pin = (pin or "").strip()
    if not PIN_RE.match(pin):
        return False, "Indiquez votre code à 4 chiffres."
    tech = get_technician_by_name(conn, name)
    if not tech:
        return False, "Technicien inconnu, contactez l'administrateur pour créer votre fiche."
    if not tech["pin_code"]:
        return False, "Aucun code n'a encore été configuré pour vous, contactez l'administrateur."
    if tech["pin_code"] != pin:
        _register_pin_failure(name)
        return False, "Code incorrect."
    _clear_pin_failures(name)
    return True, None
...
def create_ticket():
    ...
    technicien_pin = (request.form.get("technicien_pin") or "").strip()
    ...
    conn = get_conn()
    pin_ok, pin_error = verify_technician_pin(conn, technicien, technicien_pin)
    if not pin_ok:
        conn.close()
        return jsonify({"error": pin_error}), 403
...
def list_technicians():
    ...
    if USE_PG:
        cur = conn.cursor()
        cur.execute("SELECT name FROM technicians ORDER BY lower(name)")
        names = [r[0] for r in cur.fetchall()]
        cur.close()
    else:
        rows = conn.execute("SELECT name FROM technicians ORDER BY lower(name)").fetchall()
        names = [r["name"] for r in rows]
    conn.close()
    return jsonify([n for n in names if n])


def technician_to_dict(r):
    return {
        "id": r["id"],
        "name": r["name"],
        "pin_code": r["pin_code"],
...
def admin_list_technicians():
    ...
    if USE_PG:
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute("SELECT * FROM technicians ORDER BY lower(name)")
        rows = cur.fetchall()
        cur.close()
    else:
        rows = conn.execute("SELECT * FROM technicians ORDER BY lower(name)").fetchall()
    conn.close()
    return jsonify([technician_to_dict(r) for r in rows])
...
def admin_create_technician():
    ...
    if pin_code and not PIN_RE.match(pin_code):
        return jsonify({"error": "Le code doit être composé de 4 chiffres."}), 400

    conn = get_conn()
    if get_technician_by_name(conn, name):
        conn.close()
        return jsonify({"error": "Ce technicien existe déjà."}), 409
    ...
    if USE_PG:
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO technicians (id, name, pin_code, created_at) VALUES (%s, %s, %s, %s)",
            (tech_id, name, pin_code or None, created_at),
        )
        conn.commit()
        cur.close()
    else:
        conn.execute(
            "INSERT INTO technicians (id, name, pin_code, created_at) VALUES (?, ?, ?, ?)",
            (tech_id, name, pin_code or None, created_at),
        )
        conn.commit()
    conn.close()
    return jsonify({"id": tech_id}), 201


@app.route("/api/admin/technicians/<tech_id>", methods=["PUT"])
@login_required
def admin_update_technician(tech_id):
    data = request.get_json(force=True, silent=True) or {}
    ...
    if USE_PG:
        ...
        existing = conn.execute("SELECT * FROM technicians WHERE id = ?", (tech_id,)).fetchone()
    if not existing:
        conn.close()
        return jsonify({"error": "Technicien introuvable."}), 404

    name = existing["name"] if name_raw is None else name_raw.strip()
    pin_code = existing["pin_code"] if pin_raw is None else (pin_raw.strip() or None)
    if not name:
        conn.close()
        return jsonify({"error": "Le nom est obligatoire."}), 400
    if pin_code and not PIN_RE.match(pin_code):
    ...
    other = get_technician_by_name(conn, name)
    if other and other["id"] != tech_id:
        conn.close()
        return jsonify({"error": "Un autre technicien porte déjà ce nom."}), 409

    if USE_PG:
        cur = conn.cursor()
        cur.execute("UPDATE technicians SET name=%s, pin_code=%s WHERE id=%s", (name, pin_code, tech_id))
        conn.commit()
        cur.close()
    else:
        conn.execute("UPDATE technicians SET name=?, pin_code=? WHERE id=?", (name, pin_code, tech_id))
        conn.commit()
    conn.close()
    return jsonify({"ok": True})


@app.route("/api/admin/technicians/<tech_id>", methods=["DELETE"])
@login_required
