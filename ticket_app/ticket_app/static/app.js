(function(){
  ...
  function flushOfflineQueue(){
    ...
    getQueuedTickets().then(function(items){
      ...
      function next(){
        ...
        var item = items[i++];
        var fd = new FormData();
        ["technicien", "technicien_pin", "montant", "date", "categorie", "note", "pending_receipt", "card_last4", "lat", "lng", "location_label"].forEach(function(k){
  ...
  // ---------- code technicien (mémorisé par appareil) ----------
  function readPinMap(){
    try { return JSON.parse(localStorage.getItem("tt_tech_pins") || "{}"); } catch(e){ return {}; }
  }
  function writePinMap(map){
    try { localStorage.setItem("tt_tech_pins", JSON.stringify(map)); } catch(e){}
  }
  function getSavedPin(name){
    var key = (name || "").trim().toLowerCase();
    if(!key) return "";
    return readPinMap()[key] || "";
  }
  function saveSavedPin(name, pin){
    var key = (name || "").trim().toLowerCase();
    if(!key) return;
    var map = readPinMap();
    map[key] = pin;
  ...
  function refreshPinField(){
    var field = $("pinField");
    var input = $("technicienPinInput");
    var verifiedRow = $("pinVerifiedRow");
    if(!field || !input || !verifiedRow) return;
    if(getSavedPin(techName)){
  ...
  var pinChangeBtnEl = $("pinChangeBtn");
  if(pinChangeBtnEl){
    pinChangeBtnEl.addEventListener("click", function(){
      clearSavedPin(techName);
      refreshPinField();
      $("technicienPinInput").focus();
    });
  }
  var pinInputEl = $("technicienPinInput");
  if(pinInputEl){
  ...
  $("ticketForm").addEventListener("submit", function(e){
    ...
    var pinValue = getSavedPin(techName) || $("technicienPinInput").value.trim();
    if(!/^\d{4}$/.test(pinValue)){
      showToast("Indiquez votre code à 4 chiffres");
      $("technicienPinInput").focus();
      return;
    }
    ...
    fd.append("technicien_pin", pinValue);
    ...
    if(!isEdit && !navigator.onLine){
      queueOfflineTicket({
        technicien: techName, technicien_pin: pinValue, montant: montant, date: date, categorie: categorie, note: note,
        ...
        saveSavedPin(techName, pinValue);
        $("captureDialog").close();
    ...
    fetch(url, { method: isEdit ? "PUT" : "POST", body: fd })
      .then(function(res){
        if(!res.ok) return res.json().then(function(d){ throw Object.assign(new Error(d.error || "Erreur"), {status: res.status}); });
        return res.json();
      })
      .then(function(){
        saveSavedPin(techName, pinValue);
