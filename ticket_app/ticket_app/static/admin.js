(function(){
  ...
  // ---------- gestion des techniciens (codes à 4 chiffres) ----------
  var allTechnicians = [];

  function loadTechnicians(){
    return fetch("/api/admin/technicians")
      .then(function(res){
      ...
      .catch(function(){ showToast("Impossible de charger les techniciens"); });
  }

  function saveTechnician(id, name, pin, row){
    name = (name || "").trim();
    pin = (pin || "").trim();
    if(!name){ showToast("Le nom est obligatoire"); return; }
    if(pin && !/^\d{4}$/.test(pin)){ showToast("Le code doit être composé de 4 chiffres"); return; }
    fetch("/api/admin/technicians/" + id, {
      ...
      .then(function(){
        showToast("Technicien enregistré");
        loadTechnicians();
      })
  ...
  function deleteTechnician(id, name){
    if(!window.confirm("Supprimer " + name + " de la liste des techniciens ?\nSon historique de tickets sera conservé.")) return;
    fetch("/api/admin/technicians/" + id, { method: "DELETE" })
      .then(function(res){ if(!res.ok) throw new Error(); return res.json(); })
      .then(function(){
        showToast("Technicien supprimé");
        loadTechnicians();
      })
      .catch(function(){ showToast("Échec de la suppression"); });
  }

  function techAdminRow(t){
    var row = document.createElement("div");
    row.className = "tech-admin-row";

    var nameInput = document.createElement("input");
    nameInput.className = "tech-admin-name";
    nameInput.value = t.name || "";
    nameInput.setAttribute("aria-label", "Nom du technicien");
    ...
    pinInput.value = t.pin_code || "";
    pinInput.setAttribute("aria-label", "Code du technicien");
    if(!t.pin_code) pinInput.classList.add("no-pin");
    pinInput.addEventListener("input", function(){
      this.value = this.value.replace(/\D/g, "").slice(0, 4);
      this.classList.toggle("no-pin", !this.value);
    });
    row.appendChild(pinInput);

    var saveBtn = document.createElement("button");
    saveBtn.className = "btn-sm ok-btn"; saveBtn.type = "button"; saveBtn.textContent = "Enregistrer";
    saveBtn.addEventListener("click", function(){ saveTechnician(t.id, nameInput.value, pinInput.value, row); });
    row.appendChild(saveBtn);

    var delBtn = document.createElement("button");
    delBtn.className = "btn-sm bad-btn"; delBtn.type = "button"; delBtn.textContent = "Supprimer";
    delBtn.addEventListener("click", function(){ deleteTechnician(t.id, t.name); });
    row.appendChild(delBtn);

    return row;
  }

  function renderTechAdminList(){
    var list = $("techAdminList");
    list.innerHTML = "";
    if(allTechnicians.length === 0){
      list.innerHTML = '<div class="empty-state">Aucun technicien pour l\'instant — ajoutez-en un ci-dessous.</div>';
      return;
    }
    allTechnicians.forEach(function(t){ list.appendChild(techAdminRow(t)); });
  }

  $("manageTechBtn").addEventListener("click", function(){
    loadTechnicians();
    $("techniciansDialog").showModal();
  });

  $("techAddForm").addEventListener("submit", function(e){
    e.preventDefault();
    var name = $("techAddNameInput").value.trim();
    var pin = $("techAddPinInput").value.trim();
    if(!name){ showToast("Le nom est obligatoire"); return; }
    if(pin && !/^\d{4}$/.test(pin)){ showToast("Le code doit être composé de 4 chiffres"); return; }
    fetch("/api/admin/technicians", {
      ...
      .catch(function(err){ showToast(err.message || "Échec de l'ajout"); });
  });

  $("techAddPinInput").addEventListener("input", function(){
    this.value = this.value.replace(/\D/g, "").slice(0, 4);
  });
